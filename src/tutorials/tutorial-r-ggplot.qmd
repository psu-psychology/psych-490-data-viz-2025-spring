...
We'll make one more tweak before moving on.
What if we want to follow best practice and sort the bars in numeric order?

There are several ways to to this, but the way shown below uses the `forcats` package to 'mutate' or change the 'category' variable into one that is reordered by the 'value' variable.

```{r}
data_random_discrete |>
  dplyr::mutate(category = forcats::fct_reorder(category, value)) |>
  ggplot() +
  aes(x = category, y = value, fill = category) +
  geom_col(stat = "identity")
```

To reverse the order from ascending to descending, the default, we do the following:


```{r}
library(desc)
data_random_discrete |>
  dplyr::mutate(category = fct_reorder(category, desc(value))) |>
  ggplot() +
  aes(x = category, y = value, fill = category) +
  geom_col(stat = "identity")
```
